...
class ScenarioParams(ParameterSet):
    ...
    """Final year of results to report."""

    #endregion

    # Implementation Unit Characteristics
    #region
        
    soln_avg_annual_use: float = None
    """SOLUTION Average Annual Use

    The average number of functional units each SOLUTION implementation unit provides, per year. E.g. the average annual number 
    of passenger kilometers traveled per electric vehicle."""
    # excelref: SolarPVUtil "Advanced Controls"!F128

    conv_avg_annual_use: float = None
    """CONVENTIONAL Average Annual Use

    The average number of functional units each CONVENTIONAL implementation unit provides, per year. E.g. the average annual number 
    of passenger kilometers traveled per conventional vehicle."""
    # excelref: SolarPVUtil "Advanced Controls"!F95

    soln_lifetime_capacity: float = None
    """SOLUTION Lifetime Capacity
    
    The average expected number of functional units generated by a SOLUTION implementation unit throughout its lifetime before replacement is required.
    E.g. an electric vehicle will have an average number of passenger kilometers it can travel until it can no longer be used and a new vehicle is 
    required. Another example would be an HVAC system, which can only service a certain amount of floor space over a period of time before it will 
    require replacement.  <<TODO: What does this mean in Land models>>
    
    In some cases there may be no limit (or the limit is longer than the analysis period of the model), in which case the value should remain None."""
    # excelref: SolarPVUtil "Advanced Controls"!E128

    conv_lifetime_capacity: float = None
    """CONVENTIONAL Lifetime Capacity
    
    The average expected number of functional units generated by a CONVENTIONAL implementation unit throughout its lifetime before replacement is required.
    E.g. a vehicle will have an average number of passenger kilometers it can travel until it can no longer be used and a new vehicle is 
    required.
