...
class VMAReader:
    def __init__(self, wb):
        """
        wb: Workbook from an Excel file, as from openpyxl
        """
        self.wb = wb
    ...
    def read_single_table(self, source_id_cell, sheetname, fixed_summary):
        ...
        if isinstance(source_id_cell, str):  # for convenience + testing
            row1, col1 = openpyxl.utils.cell.coordinate_to_tuple(source_id_cell)
        ...
        for c in range(16):
            ...
            name_to_check = self.normalize_col_name(str(val).strip().replace('*', ''))
        ...
        for r in range(last_row, last_row + 100):  # look past last row in columns R and S
            if sheet.cell(row1 + r, 18).value == 'Use weight?':
                ...
                break
            if sheet.cell(row1 + r, 19).value == 'Use weight?':
                use_weight = tools.util.convert_bool(sheet.cell(row1 + r + 1, 19).value)
                break
        else:
    ...
    def _find_tables(self, sheetname):
        ...
        for table_num in range(1, 36):
            ...
            for rows_to_next_table in range(200):
                title_from_cell = str(sheet.cell(row + rows_to_next_table, col).value).strip()
                ...
                if table_num_on_sheet == table_num and VMA_heading_check:  # if title cell of table
                    for space_after_title in range(10):
                        ...
                        if sheet.cell(row + offset,
                                            col).value == 'SOURCE ID: Author/Org, Date, Info':
