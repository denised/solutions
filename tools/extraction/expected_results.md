# Creating the expected_results zip file
 ...
 * We need access to the results for each scenario, which means running the Excel workbook to load the scenario, which means that 
 ...
 The expected_results.zip file contains an archive that is structured as follows:
...
## Second Part: Build the expected_results.zip

Run the code in `create_expected_results.py`, either from the command line, or from within a python interpreter or Jupyter notebook
(in the latter case, the function you are looking for is `create_expected_results.create_expected_results(directory)`).

This should result in a file `expected_results.zip` being created in your directory.

The last step is to move the `expected_results.zip` to the `testdata` subdirectory of your solution directory (look at other solutions
for examples)
